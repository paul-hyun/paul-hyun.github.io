...
BERT는 단순히 Transformer Encoder를 실행합니다.  
...
4. BERT 실행결과의 첫 번째([CLS]) Token을 outputs_cls 변수에 저장합니다. (줄: 18)
5. outputs_cls를 입력으로 projection_cls를 실행하여 NSP를 예측하도록 합니다. (줄: 20)
6. BERT 실행결과를 입력으로 projection_lm을 실행하여 MLM을 예측하도록 합니다. (줄: 22)
...
다음 포스트 ‘BERT(Bidirectional Encoder Representations from Transformers) 구현하기 (2/2)’에서는 BERT를 이용해서 ‘Naver 영화리뷰 감정분석’ 과정을 정리 하겠습니다.
